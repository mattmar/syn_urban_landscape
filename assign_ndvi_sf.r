...
plot(raster(mmf[n.rows:1,]),breaks=c(0.9,1.9,2.9,3.9,4.9,5.9,6.9,7.9),col=c("brown","black","grey85","grey80","grey75","dark green","light green"),legend=FALSE,xlim=c(0,1),useRaster=TRUE,ylim=c(0,1),main="SF simulated classified",cex.main=2)

plot(raster(mmf[n.rows:1,]), legend.only=TRUE, col=c("black","blue","grey95","grey90","grey75","light green","dark green","brown"), legend.width=1, legend.shrink=0.9, axis.args=list(at=seq(1, 7, length.out=8), labels=c("Road", "River", "Small houses","Medium Houses","Big Houses","Pub Green areas","Priv Green Areas","Barren surface"), cex.axis=1),
   legend.args=list(text='', side=4, font=2, line=1, cex=1),smallplot=c(0.88,0.89, 0.14,0.87),add=T); par(mar = par("mar"))
plot(raster(mmndvi[n.rows:1,]),main="SF simulated NDVI",cex.main=2)
dev.off()

mmatrix <- matrix(1:(n.rows*n.cols), n.rows, n.cols)
a<-gdal_polygonizeR(raster(mmatrix[n.rows:1,], xmx=n.cols, ymx=n.rows),outshape="/home/matteo/Desktop/PhD/topics/urban_veg_albo/syn_matrices/syn_matrix_sf.shp")
writeRaster(sf_lulc,"~/Desktop/PhD/topics/urban_veg_albo/syn_matrices/syn_lulc_sf",format="GTiff")
writeRaster(sf_ndvi,"~/Desktop/PhD/topics/urban_veg_albo/syn_matrices/syn_lulc_ndvi",format="GTiff")
