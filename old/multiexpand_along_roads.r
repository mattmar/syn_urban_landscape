...
#Add secondary roads
mm1 <- multiexpand(mm,200,1000,0,c(0,0),10000,1,c(10,10),"px",nbr.matrix=nn2x,xnnoise=2,along=TRUE,along.value=-999)

mm2 <- matrix(1, n.rows, n.cols)
mm2[mm == -999 | mm1 > 2] <- -999
mm2[mm == -1 ] <- -1

mm3 <- multiexpand(mm2,200,1000,0,c(n.rows/2,n.cols/2),10000,1,c(10,10),"px",nbr.matrix=nn2y,ynnoise=1,along=TRUE,along.value=-999)
...
houses_road<-multiexpand(mm4,8000,cluster.size=9,0,c(n.rows/2,n.cols/2),50000,1,c(5,5),mode="ca",nbr.matrix=nn8,along=TRUE,along.value=-999)
...
houses_isolated<-multiexpand(mm6,1000,cluster.size=25,cluster.size.prob=0,start=c(100,300),count.max=100000,range=2,contiguity=c(5,5),mode="ca",nbr.matrix=nn24)
