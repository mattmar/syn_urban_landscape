...
mm4 <- multiexpand(mm3,30,150,0,c(n.rows/2,n.cols/2),10000,200,c(500,500),"px",nbr.matrix="nn2x",xnnoise=-1,along=TRUE,along.value=-999)

mm4b <- matrix(1, n.rows, n.cols)
mm4b[ mm3==-1] <- -999
mm4b[mm3 == -999] <- -1

mm8 <- multiexpand(mm4b,30,400,0,c(n.rows/2,n.cols/2),20000,100,c(500,500),"px",nbr.matrix="nn2y",ynnoise=-1,along=TRUE,along.value=-999)

mm9 <- matrix(1, n.rows, n.cols)
mm9[mm4 > 2 | mm4b == -1 | mm4b == -999 | mm8 > 2] <- -1

#Big public green areas
green_park<-multiexpand(mm9,3,cluster.size=1000,100,c(n.rows/2,n.cols/2),50000,100,c(500,500),mode="px",nbr.matrix="nn8")

mm10 <- matrix(1, n.rows, n.cols)
mm10[mm3 == -1 | mm3 == -999 | green_park > 2] <- -1
mm10[mm4 >2 | mm8 > 2] <- -999

#Add houses along the roads
houses_road<-multiexpand(mm10,10000,cluster.size=9,0,c(n.rows/2,n.cols/2),50000,1,c(0.5,0.5),mode="ca",nbr.matrix="nn8",along=TRUE,along.value=-999)
