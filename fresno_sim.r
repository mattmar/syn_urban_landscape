...
green_park<-multiexpand(mm3,3,cluster.size=1000,100,c(n.rows/2,n.cols/2),50000,100,c(100,100),mode="px",nbr.matrix="nn24")
...
houses_road_m<-multiexpand(mm11,5000,cluster.size=15,cluster.size.prob=0,start=c(n.rows/2,n.cols/2),count.max=100000,range=1,contiguity=c(0.1,0.1),mode="ca",nbr.matrix="nn14",along=TRUE,along.value=-999)

mm12 <- matrix(1, n.rows, n.cols)
mm12[mm11 == -1] <- -1
mm12[mm11 == -999 | houses_road_m >2] <- -999

houses_road_s<-multiexpand(mm12,5000,cluster.size=9,0,start=c(n.rows/2,n.cols/2),100000,1,c(0.1,0.1),mode="ca",nbr.matrix="nn8",along=TRUE,along.value=-999)
...
green_private<-multiexpand(mm13,3000,cluster.size=5,1,c(50,400),100000,5,c(50,50),mode="px",nbr.matrix="nn8",along=T,along.value=-999)
